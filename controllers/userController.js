...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (existingUser)
      return res
        .status(400)
        .json({ mensaje: "El usuario ya existe en el servidor" });
    ...
    res
      .status(500)
      .json({ mensaje: "Se produjo un error al registrar el usuario" });
  }
};
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (!user)
      return res.status(404).json({ mensaje: "el usuario no es encontrado" });
...
const deleteUser = async (req, res) => {
  ...
  try {
    ...
    if (!deletedUser) {
      return res
        .status(404)
        .json({ mensaje: "no se pudo encontrar el usuario" });
    }
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!user)
      return res
        .status(400)
        .json({ mensaje: "Las credenciales son invalidas" });
...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ mensaje: "El email se ha enviado de forma corecta" });
  } catch (error) {
...
const resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user)
      return res.status(400).json({ mensaje: "Token incorrecto o expirado" });
    ...
    res
      .status(200)
      .json({ mensaje: "La contraseña se actualizo correctamente" });
  } catch (error) {
